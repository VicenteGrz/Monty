...
<body>
    ...
    <script>
        ...
        // Función para obtener las coordenadas de un lugar usando Mapbox
        async function getPlaceCoordinates(place) {
            const apiUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(place)}.json?access_token=${mapboxgl.accessToken}&limit=5`;
            try {
                const response = await fetch(apiUrl);
                const data = await response.json();
                return data.features.map(item => ({
                    name: item.place_name,
                    lat: item.center[1],  // Latitud está en la segunda posición
                    lon: item.center[0]   // Longitud está en la primera posición
                }));
            } catch (error) {
        ...
        async function getDirections(userLat, userLon, placeLat, placeLon) {
            ...
            try {
                ...
                console.error("Error al obtener las indicaciones de ruta", error);
        ...
        recognition.addEventListener('result', async (event) => {
            ...
            try {
                ...
                if (places.length > 0) {
                    ...
                    if (closestPlace) {
                        coordinatesElement.textContent = `Resultado más cercano: ${closestPlace.name} (Lat: ${closestPlace.lat}, Lon: ${closestPlace.lon})`;
